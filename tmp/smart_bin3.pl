...
mark_next_level(var_Current):-next_level(var_Current,var_Next),retractall(fill_level(var__)),assert(fill_level(var_Next)).

evi(auto_fill(dummy)):-clause(agent(var_BinID),var__),current_level(var_Level),(clause(full_alert_sent,var__),var_Level=100->a(message(control_center,inform(full(var_BinID),var_BinID)));format('[~w] Filling: ~w%~n',[var_BinID,var_Level]),a(message(logger,inform(status(var_BinID,var_Level),var_BinID))),(var_Level=100->(clause(full_alert_sent,var__)->true;format('[~w] FULL reached -> alerting control_center~n',[var_BinID]),a(message(control_center,inform(full(var_BinID),var_BinID))),assert(full_alert_sent));mark_next_level(var_Level))).

auto_fill(dummy).

normalize_cleared(inform(cleared,var__From)).

normalize_cleared(inform(cleared,var__Meta,var__From)).

normalize_cleared(inform(cleared)).

normalize_cleared(cleared).

handle_cleared_message(var_Msg):-normalize_cleared(var_Msg),clause(agent(var_BinID),var__),retractall(fill_level(var__)),assert(fill_level(0)),retractall(full_alert_sent),format('[~w] Reset to 0%~n',[var_BinID]),a(message(logger,inform(reset(var_BinID),var_BinID))).

eve(inform_E(var_Msg)):-handle_cleared_message(var_Msg).

eve(inform_E(var_Msg,var__From)):-handle_cleared_message(var_Msg).

eve(inform_E(var_Msg,var__Meta,var__From)):-handle_cleared_message(var_Msg).
...
trigger_inform_handlers(var_X,var_M,var_Ag):-catch(call(eve(inform_E(var_X,var_Ag))),_346429,true),catch(call(eve(inform_E(var_X,var_M,var_Ag))),_346457,true),catch(call(eve(inform_E(var_X))),_346481,true).
...
call_cfp(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_346193,var_Ontology,_346197),_346187),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_cfp(var_A,var_C,var_Ag,_346231)),a(message(var_Ag,propose(var_A,[_346231],var_AgI))),retractall(ext_agent(var_Ag,_346269,var_Ontology,_346273)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_346067,var_Ontology,_346071),_346061),asserisci_ontologia(var_Ag,var_Ontology,var_A),once(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,accept_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_346137,var_Ontology,_346141)).

call_propose(var_A,var_C,var_Ag):-clause(agent(var_AgI),var__),clause(ext_agent(var_Ag,_345955,var_Ontology,_345959),_345949),not(call_meta_execute_propose(var_A,var_C,var_Ag)),a(message(var_Ag,reject_proposal(var_A,[],var_AgI))),retractall(ext_agent(var_Ag,_346011,var_Ontology,_346015)).
...
call_cancel(var_A,var_Ag):-if(clause(high_action(var_A,var_Te,var_Ag),_345519),retractall(high_action(var_A,var_Te,var_Ag)),true),if(clause(normal_action(var_A,var_Te,var_Ag),_345553),retractall(normal_action(var_A,var_Te,var_Ag)),true).
...
refused_message(var_To,var_M):-clause(eliminated_message(var_M,var_To,motivation(conditions_not_verified)),_345335).
...
send_jasper_return_message(var_X,var_S,var_T,var_S0):-clause(agent(var_Ag),_345183),a(message(var_S,send_message(sent_rmi(var_X,var_T,var_S0),var_Ag))).

gest_learn(var_H):-clause(past(learn(var_H),var_T,var_U),_345131),learn_if(var_H,var_T,var_U).

evi(gest_learn(var_H)):-retractall(past(learn(var_H),_345007,_345009)),clause(agente(_345029,_345031,_345033,var_S),_345025),name(var_S,var_N),append(var_L,[46,112,108],var_N),name(var_F,var_L),manage_lg(var_H,var_F),a(learned(var_H)).

cllearn:-clause(agente(_344801,_344803,_344805,var_S),_344797),name(var_S,var_N),append(var_L,[46,112,108],var_N),append(var_L,[46,116,120,116],var_To),name(var_FI,var_To),open(var_FI,read,_344901,[]),repeat,read(_344901,var_T),arg(1,var_T,var_H),write(var_H),nl,var_T==end_of_file,!,close(_344901).
