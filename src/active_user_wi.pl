...
read_server_endpoint(I):-
  ( read_server_endpoint_from('server.txt', I)
  ; read_server_endpoint_from('../server.txt', I)
  ; read_server_endpoint_from('src/server.txt', I)
  ), !.

read_server_endpoint_from(Path, I):-
  catch((open(Path,read,Stream,[]), read(Stream,I), close(Stream)), _, fail),
  I \= end_of_file.
